...
/**
 * A heuristic variant of breadth-first-search, sorting the entire queue when a vertex is expanded. The sorting order is
 * defined by the specified vertex cost function.
 * <p>
 * From: Patrick Henry Winston, Artificial Intelligence, Addison-Wesley, 1984
 * 
 * @author Armin Reichert
 */
public class BestFirstSearch extends AbstractGraphSearch {

	private final ToDoubleFunction<Integer> fnEstimatedCost;

	/**
	 * Creates a best-first traversal instance for the given graph and vertex cost estimation and uniform edge cost.
	 * 
	 * @param graph           a graph
	 * @param fnEstimatedCost estimated vertex cost. Queue will always be sorted by increasing priority.
	 */
	 ...
	 * 
	 * @param graph           a graph
	 * @param fnEstimatedCost vertex priority function. Queue will always be sorted by increasing priority.
	...
	public BestFirstSearch(Graph<?, ?> graph, ToDoubleFunction<Integer> fnEstimatedCost,
		...
		this.fnEstimatedCost = Objects.requireNonNull(fnEstimatedCost);
		this.frontier = new MinPQVertexQueue(fnEstimatedCost);
	}
